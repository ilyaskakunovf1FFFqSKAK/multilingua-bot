...
async def get_phone(message: Message, state: FSMContext):
    try:
        ...
        # безопасно достаём данные (чтобы не падало KeyError)
        ...
        if ADMIN_CHAT_ID != 0:
            await bot.send_message(ADMIN_CHAT_ID, lead_text)  # без Markdown, чтобы не ломалось
        ...
        await message.answer("Произошла ошибка при обработке заявки ❌. Я сообщил разработчику (то есть тебе 😄).")
    ...
    if ADMIN_CHAT_ID != 0:
        await bot.send_message(ADMIN_CHAT_ID, lead_text, parse_mode="Markdown")
        await message.answer("Спасибо! Заявка отправлена ✅ Мы свяжемся с вами в ближайшее время.")
    else:
        await message.answer(
            "Заявка собрана ✅\n"
...
# ✅ Работает в группах и с /chatid@BotName
@dp.message(Command("chatid"))
async def chatid(message: Message):
    await message.answer(f"chat_id этого чата: `{message.chat.id}`", parse_mode="Markdown")
